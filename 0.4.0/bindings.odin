/*
Copyright (c) 2025 A-Boring-Square & Contributors
...
WREN_VERSION_NUMBER :: VERSION_MAJOR * 1000000 + VERSION_MINOR * 1000 + VERSION_PATCH
...
//
// - To allocate new memory, [memory] is nil and [newSize] is the desired
//   size. It should return the allocated memory or nil on failure.
//
...
Configuration :: struct {
  ...
  //
  // If `nil`, defaults to a built-in function that uses `realloc` and `free`.
  ...
  // stack traces.
  //
  // If you leave this function nil, then the original import string is
  // treated as the resolved string.
  //
  // If an import cannot be resolved by the embedder, it should return nil and
  ...
  //
  // If the foreign function could not be found, this should return nil and
  ...
  //
  // If this is `nil`, Wren discards any printed text.
  writeFn: WriteFn,

  // The callback Wren uses to report errors.
  //
  // When an error occurs, this will be called with the module name, line
  // number, and an error message. If this is `nil`, Wren doesn't report any
  // errors.
  errorFn: ErrorFn,
...
when ODIN_OS == .Windows {
    when ODIN_DEBUG {
        when ODIN_ARCH == .amd64 {
            foreign import wren_lib "bin/Windows/Debug/x86_64/wren_d.lib"
        }
        when ODIN_ARCH == .i386 {
            foreign import wren_lib "bin/Windows/Debug/x86/wren_d.lib"
        }
        else {
            #panic("Unsuported arch")
        }
    }
    else {
        when ODIN_ARCH == .amd64 {
            foreign import wren_lib "bin/Windows/Release/x86_64/wren.lib"
        }
        else when ODIN_ARCH == .i386 {
            foreign import wren_lib "bin/Windows/Release/x86/wren.lib"
        }
        else {
            #panic("Unsuported arch")
        }
    ...
    when ODIN_DEBUG {
        when ODIN_ARCH == .amd64 {
           foreign import wren_lib "bin/Linux/Debug/x86_64/libwren_d.a"
        }
        else when ODIN_ARCH == .i386 {
            foreign import wren_lib "bin/Linux/Debug/x86/libwren_d.a"
        }
        else {
            #panic("Unsuported arch")
        }
        ...
        when ODIN_ARCH == .amd64 {
            foreign import wren_lib "bin/Linux/Release/x86_64/libwren.a"
        }
        when ODIN_ARCH == .i386 {
            foreign import wren_lib "bin/Linux/Release/x86/libwren.a"
        }
        else {
            #panic("Unsuported arch")
        }
...
foreign wren_lib {
    ...
    // freed after calling this. If [configuration] is `nil`, uses a default
    // configuration.
    ...
    // captured using GetSlotHandle(), which ensures the garbage collector can
    // find them.
    //
    ...
    // space, you can call EnsureSlots() to add more.
    //
    ...
    // use `HasModule()` to ensure that before calling.
